...
- **希望スタイル**: タスクに応じてエージェントを召喚し、自律的に作業を進める
- **役職**: 事務職
- **主な用途**: 日本語で指示してPythonコードを生成（ノーコード的な使い方）
- **技術レベル**: 初心者〜中級（基本的なコードは読める）
- **よく使うツール**: PowerPoint、Excel等のOfficeソフト
- **年齢**: 55歳
- **興味・関心**: 転職、新規起業、エンジェル投資
- **調査依頼**: 上記3テーマについてエージェントによる定期調査を希望
- **調査エリア**: 富山県高岡市（エリア限定）
- **転職・起業**: 具体的なアイデアなし（情報収集・探索段階）
- **エンジェル投資**: 国内スタートアップ企業が対象、試験的投資額は約50万円
- **現在の年収**: 約800万円（総額）
- **調査結果保存先**: `reports/` フォルダにMarkdown形式で保存
- **調査頻度**: 毎日・前日との差分（新情報）を提案
- **健康状態**: 歩行が苦手なため、リモートワーク・在宅勤務・在宅起業を優先的に検討
