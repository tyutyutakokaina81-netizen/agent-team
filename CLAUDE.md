...
各役職フォルダ直下の `知恵帳.md` に、**毎日1件ずつ**学びを追記する。

- 対象: CDO / CFO / CMO / CPO / CSO（今後増える役職も同様）
- 形式: 日付見出し → 「学び」「背景」「示唆」の3点を短く
- 1日1件で十分。書けない日は無理に書かない（実態を歪めない）
- 顧客個人を特定する情報はここに書かない（センシティブは `research/` 側）
- 役職を跨いで共有したい知恵は、引用元の知恵帳パスをリンクで残す
