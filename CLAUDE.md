...
2. **デフォルト動作 = 今日の note 記事を1本作成**（CMO役職）
   ...
   - 本文はそのまま貼れる完成稿。海外読者にも届くよう **英語要約を併記**
   - サムネ：実写は生成不可 →「実写生成プロンプト」＋「デザインPNG」で代替
3. **保存**：`CMO/outputs/YYYY-MM-DD_note記事_*.md`（＋サムネ素材）
4. **記憶を更新**：`CMO/_index.md` の成果物ログと `context/STATE.md`（最新状態・次にやること・決定ログ）を必ず更新する
5. **無確認で自動 commit & push**（下記ポリシー）
6. 完了報告（要点 → 詳細 → 次アクション）
