...
| CQO | 品質保証・検品・パイプライン生存監視（2026-09-05 新設） | 検品結果、異常の一次診断、要件監視 |
| CGO | グロース・海外流入導線の設計と保守（2026-09-05 新設） | SEO基盤、内部リンク、被リンク施策 |
...
### 経営哲学＝意思決定の背骨（溝口勇児式・全役職/全セッション適用）
正本：`docs/management-philosophy.md`（5つの柱＝選択/成長/運と縁/解釈/勇気）。運用OSは`context/STATE.md`。

- **選択**：曖昧な目標を禁止＝**数字と期限に落ちていない計画は計画ではない（解像度）**。全部やる＝何もやらない＝**トレードオフ（勝ち筋1本に集中）**。掲げたゴール（8/1初売上／10月末月¥10,000）を常に念頭に。
- **成長**：できない理由から入らない（心のサイドブレーキを引かない）。3問フィルタを通ったら即実行。無風の日を作らない（毎朝の自動実行＋週次の数字レビューを止めない）。
- **運と縁**：試行回数だけが確率を買う（毎日×世界配布）。読者に先に与えるGIVER。嘘つきは運と縁を逃す＝A5/事実検証は倫理かつ長期投資。
- **解釈**：コントロールできる側（コード・手順・打ち手）だけ直す＝**自責**（UI変更や検索アルゴリズムを嘆かない）。不都合な数字（フォロワー/売上0）から目を逸らさない。
- **勇気**：失敗は記録して次に使う消耗品（感情処理でなく構造3問＝何が/なぜ/次どう変える）。実機検証なき完了宣言禁止・URL報告なき公開禁止（高い要求水準）。
- ※炎上型アテンション・強引な組織運営は採らない（A5とブランド保護が上位）。
...
| A3 | **サムネはオーナー/cloudが取得（写真風で統一）** | フリー素材/画像AI/Canva 等。形式は全部「写真風」。code は記事に `[写真X]` placeholder を置くだけ。**全記事に必ず付ける**（2026-09-23確定・上の「実行して」プロトコル参照） |
...
2. **対象日の特定**：`CMO/outputs/` で 2本揃っていない最も近い日付を対象に
   - 2本未満 → 不足分を作成して 2本に揃える／2本揃っている → 次の未完成日へ
3. **デフォルト動作 = 対象日分の note 記事を 2 本作成**（CMO主導・CAOが事前選定）
   - **本数：2本厳守**（少なくも多くもない）／**2本のうち1本は食**（オーナー確定 2026-09-23）
     ※旧「5本厳守」から変更。実績はずっと2本/日で、記載だけが5本のまま残っていた。
       1本ずつサムネを確認して出せる本数がこれ、という理由で2本に確定した。
   - **同一カテゴリ2日連続を回避**／一次観察（高岡住人視点）を優先
   - 本文はそのまま貼れる完成稿。**英語要約を併記**（記事ごとに別パターン）
   - 記事末尾に **「事実検証ノート」**（✅確実/⚠️要確認/諸説あり）を必ず付ける
   - **`## ハッシュタグ` ブロックを必ず付ける（5〜8個）**。note ではタグがタグページ・おすすめ経由の
     主要な発見経路で、無いと**タグ0個で公開され reach が直接落ちる**。
     2026-08-22〜09-18 に書いた約25本はこれが抜けていた（publisher はブロックの有無でしか判断しない）。
     構成の目安＝**題材語／富山（＋高岡・氷見など市レベル）／カテゴリ（食・暮らし等）／季節／`#japan` `#Toyama`**。
     英語タグを必ず1〜2個入れる＝North Star が海外読者のため。
     ※`ops/check_requirements.py` の **R23** が公開キューを見て、タグ無し・5個未満なら BROKEN を出す
   - **サムネ：全記事に必ず付ける（オーナー確定 2026-09-23）。**
     note の見出し画像は**公開したあとから付けられない**ので、キューに入れる前に決める（R32）。
     判定基準＝**題材が合っていれば採用する**。ぴったりの構図でなくてよい
     （例：白菜の記事に畑で育っている株の写真＝採用。ずれている点は事実検証ノートに書く）。
     **落とすのは「明らかに別物」のときだけ**（そば記事に乾麺の束／豚汁で椀に具が見えない／
     本文と違う器具・生き物の別の成長段階など）。落としたら**必ず語を変えて取り直す**。
     **ただし次の3つは、題材が合っていても必ず落とす**（2026-09-23 に実例が出た）：
       ① **人物の顔が写っているもの**（とくに子ども）＝A4・肖像。獅子舞の写真がこれで落ちた。
       ② **店名・業者名・電話番号などの連絡先が読めるもの**＝衣替えで「桐タンス更生」の
          広告看板（電話番号入り）が来た。
       ③ **他県の特定施設が主題のもの**＝用水路で「琵琶湖疏水記念館」の建物が来た。
          （風景として他県が写るのは可。**施設名が主題**なのが駄目）
       ④ **個人名が読める物**＝怪談で永代提灯（奉納者名が何十個も読める）が来た。A4。
       ⑤ **絵画・イラスト**＝日焼けでゴッホの油彩が来た。A3＝写真風で統一。
       ⑥ **政治的な掲示物**＝回覧板で政党ポスター（候補者2名の顔＋住所表示）が来た。
          顔だけでなく、**政治的立場を示すものは題材が合っていても使わない**（ブランド保護）。
       ⑦ **加工・合成された画像**＝障子でドロステ効果の渦巻き合成が来た。実写ではない。
     ※①〜⑦は「題材が合っていても落とす」型。**落としたら必ず語を変えて取り直す**（無サムネにしない）。
     ※`fetch_thumbnails_wikimedia.py` の `UNUSABLE_NAME_HINTS` が、
       ファイル名の時点で分かるもの（商品パッケージ/奉納物/便所/ホテル名入り）を先に落とす。
     ※従来 code が使っていた「誤サムネより無サムネが正」は**廃止**。9月の30本中11本が
       無サムネで出ており、これは指示（サムネつきで毎日）に反していた。
     ※`_no_auto`（無サムネ確定）は**原則使わない**。使うのは公開済みで後から付けられない記事だけ。
   - **マガジン：記事情報に `- マガジン: 高岡の食／北陸の暮らし` の行を必ず書く（2026-09-23 追加）。**
     publisher がこの行を読んで、公開の瞬間に note のマガジンへ入れる。
     既存のマガジンは5つ＝**高岡の食 / English Articles / 高岡・富山を歩く / 仕事とAI / 北陸の暮らし**。
     1記事あたり1〜2つ。**`English Articles` には自動で入れない**（英語版投稿用で判断が要る）。
     ※これが無いと**どのマガジンにも入らずに公開**され、後から1本ずつブラウザで入れる作業になる。
       実際、5つ作ってあるのに **196本が1本も入っていなかった**（編成リストは 2026-06-29 に
       できていたが、オーナーが手でクリックする前提だったので86日間動かなかった）。
     ※`ops/check_requirements.py` の **R32** がキューを見て、指定が無ければ BROKEN を出す。
...
> ⚠️ 自動公開には安全ガードあり（未来日付ブロック・テスト記事フィルタ・重複チェック・1日最大5本(上限)・
