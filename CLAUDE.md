...
## 「実行して」プロトコル v2（自動実行・オーナー承認済み・2026-05-31 更新）

オーナーが **「実行して」**（または「実行」「今日の分」「今日の記事書いて」など）と言ったら、
以下を **承認不要で自動実行** する。

1. **記憶を読む**：`context/STATE.md` と `CMO/_index.md` を最初に確認する（SessionStartフックが起動時に STATE.md を自動ロード済み）
2. **対象日の特定**：CMO/outputs/ に対象日のファイルが
   - 5本揃っていない → その日を対象に**不足分を作成**して5本に揃える
   - 既に5本揃っている → **次の未完成日（5本未満）**を対象にする
3. **デフォルト動作 = 対象日分の note 記事を 5 本作成**（CMO役職主導・CAOが事前選定）
   - **本数：5本厳守**（少なくも多くもない）
   - **配分：5本中2本は食**（オーナー指示）
   - **同一カテゴリ2日連続を回避**
   - 本文はそのまま貼れる完成稿。海外読者にも届くよう **英語要約を併記**（記事ごとに別パターン）
   - 一次観察があれば優先（オーナー高岡住人視点）
   - サムネ：実写は生成不可 →「実写生成プロンプト」or オーナー写真使用前提
4. **保存**：`CMO/outputs/YYYY-MM-DD_note記事_*.md`（＋サムネ素材）
5. **記憶を更新**：`CMO/_index.md` の成果物ログと `context/STATE.md`（最新状態・次にやること・決定ログ）を必ず更新する
6. **無確認で自動 commit & push**（下記ポリシー）
7. 完了報告（要点 → 詳細 → 次アクション）

### 品質基準（オーナー指摘「適当」を反省・必須遵守）
- ❌ テンプレ的フック「Xには、Yがある」連発禁止
- ❌ 同日内で結論が重複する記事NG
- ❌ 「世界唯一」など根拠薄い断定NG
- ❌ 英語要約は記事ごとに別パターン（"If you make things..." 反復禁止）
- ❌ 個人特定情報（具体的な地区名・町名・住所）禁止
- ✅ 高岡関連は住人一人称で書く（市レベルまでの一般化はOK）

### このプロトコル時の push ポリシー（上記「必ず事前に確認する操作」の例外）
- 「実行して」プロトコルの実行に伴う **commit & push は無確認で自動実行してよい**（オーナー承認済み）。
- 理由：コンテナは使い捨てで、push しない限り成果物・記憶が消えるため（再発防止）。
- ただし **削除・上書き・git reset 等の破壊的操作は例外に含めない**（従来どおり要確認）。

### 環境の絶対制約（毎回前提にする）
- Web/リモートの使い捨てコンテナ。永続するのは **コミット済みリポジトリ内ファイルのみ**。
- note への **自動公開は不可**（認証・API・ブラウザ操作が無い）→ 公開はオーナーが手動。
