...
# ---------- CI（GitHub Actions）無人モード設定 ----------
# NOTE_CI=1 のとき: ヘッドレスChromium ＋ メール/パスワード自動ログイン。
# 認証情報は GitHub Secrets から環境変数で渡す（リポジトリには保存しない）。
# 注意: note 側の bot 検知・captcha・2段階認証が出ると CI からは突破できない。
IS_CI = os.environ.get("NOTE_CI") == "1"
NOTE_EMAIL = os.environ.get("NOTE_EMAIL", "")
NOTE_PASSWORD = os.environ.get("NOTE_PASSWORD", "")
CI_PROFILE_DIR = Path(os.environ.get(
    "NOTE_PROFILE_DIR", str(Path.home() / ".note_publisher_profile_ci")))
...
def _launch_ci(playwright):
    """CI(GitHub Actions)用: ヘッドレスChromiumを起動する。"""
    CI_PROFILE_DIR.mkdir(parents=True, exist_ok=True)
    print("🤖 CIモード: ヘッドレスChromiumを起動します")
    return playwright.chromium.launch_persistent_context(
        user_data_dir=str(CI_PROFILE_DIR),
        headless=True,
        args=[
...
def auto_login(page):
    """CI用: メール/パスワードで note にログインする。
    captcha・2段階認証が出ると突破できないため、その場合は明示的に失敗させる。"""
    if not (NOTE_EMAIL and NOTE_PASSWORD):
        sys.exit("✗ NOTE_EMAIL / NOTE_PASSWORD が未設定です（CI自動ログイン不可）。GitHub Secrets を設定してください。")
    print("🔑 CI自動ログインを試行します ...")
    page.goto(NOTE_LOGIN_URL, wait_until="domcontentloaded")
    page.wait_for_timeout(2500)
    try:
        page.locator(
            'input[type="email"], input[name="email"], input[placeholder*="メール"]'
        ).first.fill(NOTE_EMAIL)
        page.locator(
            'input[type="password"], input[name="password"], input[placeholder*="パスワード"]'
        ).first.fill(NOTE_PASSWORD)
        page.locator(
            'button:has-text("ログイン"), button[type="submit"]'
        ).first.click()
        page.wait_for_timeout(6000)
    except Exception as e:
        sys.exit(f"✗ 自動ログイン操作に失敗: {e}")
    if "login" in page.url or "accounts.google.com" in page.url:
        sys.exit("✗ ログイン後もログイン画面のまま。captcha/2段階認証/メール認証の可能性。CIからは突破不可。")
    print("✅ CI自動ログイン成功")


def load_context(playwright):
    """保存済み永続プロファイルでブラウザを起動（本物Chrome優先）。CIではヘッドレス。"""
    if IS_CI:
        return _launch_ci(playwright)
