...
# 2026-09-07 表記ゆれ辞書（CQO指摘・高1／2026-09-06「黒作り vs 黒造り」の教訓の機構化）。
# トークンは文字列一致で比べるため、**同じ題材の別表記は重複として検出できなかった**。
# 実例: 未公開ドラフト「富山のころ柿」と新稿「干し柿」は同一題材だが、文字が重ならず素通りする。
# 左を右に正規化してから比較する。新しい表記ゆれに気づいたらここに足す（1行=1組）。
TOPIC_SYNONYMS = {
    "ころ柿": "干し柿", "枯露柿": "干し柿", "つるし柿": "干し柿",
    "黒作り": "黒造り", "くろづくり": "黒造り",
    "ますのすし": "鱒寿司", "マスノスシ": "鱒寿司", "鱒ずし": "鱒寿司",
    "ホタルイカ": "蛍烏賊", "ほたるいか": "蛍烏賊",
    "シロエビ": "白えび", "白エビ": "白えび", "しろえび": "白えび",
    "ノドグロ": "のどぐろ", "喉黒": "のどぐろ",
    "ブラックラーメン": "富山ブラック",
    "初冠雪": "冠雪",
    # 2026-09-07 追加(CQO指摘・中1)。純ひらがなの主題語は _topic_tokens の
    # 「漢字を含む or 3字以上カタカナ」条件で落ちるため、辞書に載せないと重複判定に一切現れない。
    # 実測: 「ぎんなん：街路樹に実る、拾って食べるもの」のトークンは {街路樹に実る, 拾って食べる} だけだった。
    "ぎんなん": "銀杏", "ギンナン": "銀杏", "いちょう": "銀杏", "イチョウ": "銀杏", "公孫樹": "銀杏",
    "消雪パイプ": "消雪", "融雪": "消雪", "流雪溝": "消雪", "消雪": "消雪",
    # 2026-09-11 追加。R15（主題語が重複ゲートに載っているか）が新規記事で即座に検知した。
    # 「なめこ：ぬるぬるしているほうが、いい」はトークンが**空集合**になり、重複判定に一切現れなかった。
    "なめこ": "なめこ", "ナメコ": "なめこ", "滑子": "なめこ",
    "防災行政無線": "防災無線", "防災無線": "防災無線", "屋外拡声器": "防災無線", "夕方のチャイム": "防災無線",
    # 2026-09-12 追加。R15（主題語が重複ゲートに載るか）を通すため、新記事の主題語は必ずここに入れる。
    "さつまいも": "焼き芋", "サツマイモ": "焼き芋", "薩摩芋": "焼き芋", "焼き芋": "焼き芋", "焼きいも": "焼き芋",
    "紅葉": "紅葉", "もみじ": "紅葉", "こうよう": "紅葉", "ナナカマド": "紅葉",
    # 2026-09-13 追加（新記事の主題語は必ず登録＝R15を通す）
    "秋刀魚": "秋刀魚", "さんま": "秋刀魚", "サンマ": "秋刀魚",
    "水力発電": "水力発電", "水力発電所": "水力発電",
    # 2026-09-13(CQO指摘・中9): 当初「電気」を登録したが、_topic_tokens は辞書キーを
    # **タイトル全体への部分文字列**で引くため、将来の「太陽光発電」「静電気」「電気自動車」
    # 「風力発電」まで水力発電と衝突し、**公開が止まる**（topic_conflict は公開を中断する）。
    # 「既存記事に誤マッチしない」の確認は後ろ向きだけで、前方向の誤爆を見ていなかった。
    # 主題語ではなくタイトル固有句を登録して、R15を通しつつ誤爆を避ける。
    "冬に降った雪でできている": "水力発電",
    # 2026-09-14 追加。2字語（仏壇/神棚）は _topic_tokens の len<3 で落ちるため辞書登録が必須。
    # 前方向の誤爆確認（CQO指摘・中9の再発防止）: 「おはぎ」「ぼたもち」「仏壇」「神棚」「位牌」は
    # いずれも部分文字列として無関係な題材に現れない（「彼岸」は既存の彼岸花と衝突するので登録しない）。
    "おはぎ": "おはぎ", "ぼたもち": "おはぎ", "ボタモチ": "おはぎ", "牡丹餅": "おはぎ", "御萩": "おはぎ",
    "仏壇": "仏壇", "位牌": "仏壇", "神棚": "神棚",
    # 2026-09-15 追加。里芋は2字で len<3 に落ちるため登録が必要。
    # 前方向の誤爆確認: 「里芋」「さといも」「用水路」「流雪溝」はいずれも無関係な題材の
    # 部分文字列にならない（「用水」単体は「用水池」等に当たりうるので登録しない）。
    "里芋": "里芋", "さといも": "里芋", "サトイモ": "里芋", "親芋": "里芋", "子芋": "里芋",
    "用水路": "用水路", "農業用水": "用水路", "流雪溝": "流雪溝",
    # 2026-09-16 追加。**「蕎麦」は登録しない**＝既出「福井まで辛い蕎麦を食べに行く話」と衝突して
    # 公開が止まる（CQO中9で学んだ前方向の誤爆）。タイトル固有の「新蕎麦」だけ登録する。
    # 「とんぼ」も登録しない＝将来の「とんぼ玉」（富山はガラスの街）と衝突するため。
    "新蕎麦": "新蕎麦", "しんそば": "新蕎麦",
    "赤とんぼ": "赤とんぼ", "アキアカネ": "赤とんぼ", "赤トンボ": "赤とんぼ",
    # 2026-09-17 追加。あけびは純ひらがな3字で _topic_tokens の条件（漢字含む or 3字以上カタカナ）に
    # 落ちるため登録が必要。前方向の誤爆確認: 「あけび」「アケビ」「木通」「運動会」はいずれも
    # 無関係な題材の部分文字列にならない（「運動」単体は「運動不足」等に当たるので登録しない）。
    # 「木通」も外した＝機械確認で「木通しの良い部屋」に誤マッチした。実在しにくい語ではあるが、
    # 誤マッチのコストは**公開が止まる**ことなので、あけび/アケビだけで足りる以上登録しない。
    "あけび": "あけび", "アケビ": "あけび",
    "運動会": "運動会", "体育大会": "運動会", "体育祭": "運動会",
    # 2026-09-18 追加。松茸は2字で len<3 に落ちるため登録が必要。
    # 前方向の誤爆確認済み: 「松茸」「マツタケ」「稲刈り」はいずれも無関係な題材の部分文字列にならない。
    # 「稲」単体は登録しない（「稲妻」「稲荷」に当たる）。「刈り」も登録しない（「草刈り」「髪を刈る」に当たる）。
    "松茸": "松茸", "マツタケ": "松茸",
    "稲刈り": "稲刈り", "コンバイン": "稲刈り",
    # 2026-09-19 追加。**正規形を「いくら」にしてはいけない**＝この辞書は最後に
    # `{正規形: 正規形}` を自動で足すため、正規形に置いた時点で「いくら」がキーになり、
    # 「いくらでも食べられる」「いくらかかるのか」に誤マッチして**公開が止まる**（機械確認で再現）。
    # 正規形を「筋子」にすれば、記事タイトルの「生の筋子」から拾えて誤爆もしない。
    "すじこ": "筋子", "イクラ": "筋子",
    # 金木犀は3字＋漢字なので登録しなくても自然にトークン化される（登録は不要）。
    "キンモクセイ": "金木犀", "きんもくせい": "金木犀",
}
# 正規形そのものも部分一致で拾えるようにする（「黒作り」だけ辞書にあって「黒造り」が無いと、
# 片側しか正規化されず結局すれ違う）。値を自分自身へ写す組を足しておく。
TOPIC_SYNONYMS.update({v: v for v in list(TOPIC_SYNONYMS.values())})


def _canon_topic(tok: str) -> str:
    """題材トークンを表記ゆれ辞書で正規化する。"""
    return TOPIC_SYNONYMS.get(tok, tok)


def _topic_tokens(title: str) -> set:
    """タイトルから識別性のある題材トークン(漢字含む語 or 3字以上カタカナ語)を抽出。
    句読点・空白・記号で分割→末尾助詞を除去→ストップワード/短語を除外→表記ゆれを正規化。"""
    ...
    for seg in segs:
        ...
        if re.search(r"[一-鿿]", seg) or re.fullmatch(r"[ァ-ヶー]{3,}", seg):
            toks.add(seg)
    # 部分文字列としての表記ゆれも拾う（「富山のころ柿」→トークン「富山のころ柿」ではなく
    # 分割後に「ころ柿」が出ないケースがあるため、タイトル全体からも辞書引きする）
    for variant, canon in TOPIC_SYNONYMS.items():
        if variant in title:
            toks.add(canon)
    return {_canon_topic(t) for t in toks}
