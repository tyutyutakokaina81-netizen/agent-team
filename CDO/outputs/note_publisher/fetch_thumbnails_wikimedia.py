...
JP_QUERY = [
    ...
    ("松茸", ["マツタケ", "松茸", "Tricholoma matsutake", "マツタケ 収穫"]),
    ("名前だけ知っている", ["マツタケ", "Tricholoma matsutake"]),
    ("稲刈り", ["稲刈り", "コンバイン 稲刈り", "稲刈り後の田", "刈り取り 水田"]),
    ("うるさい日", ["稲刈り", "コンバイン 稲刈り"]),
    ("あけび", ["アケビの実", "Akebia quinata", "アケビ 熟", "アケビ"]),
    ("中身だけ食べる実", ["アケビの実", "アケビ 熟"]),
...
def main() -> None:
    ...
    for stem, jp, en, out in queue:
        ...
        if not tried:
            print(f"  ✗ {stem}  (土地名を含むクエリが無い → JP_QUERY に「{_place_mismatch(stem, '')}」入りの語を登録すること)")
            fail += 1
            continue
        data = None
        used = None
