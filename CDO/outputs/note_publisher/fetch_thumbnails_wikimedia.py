...
JP_QUERY = [
    ...
    #   題材が一意に決まる語だけを並べる。
    # 2026-09-23 取りこぼし回収分。公開済みで無サムネだった記事のうち、検索語が無かったもの。
    #   「二百十日」「FIRE」はCommonsに当たる語ではないので、**記事が見せている物**に寄せる。
    ("二百十日", ["稲穂", "田んぼ 稲", "水田 稲穂", "台風"]),
    ("富山の8月", ["夏祭り", "盆踊り", "提灯", "屋台 祭り"]),
    ("夏祭り", ["夏祭り", "盆踊り", "提灯"]),
    ("稲穂", ["稲穂", "水田 稲穂", "田んぼ 稲"]),
    # 2026-09-29分。単語に寄せる方針は据え置き。
    #   白菜＝「白菜」は一意（他の意味に取られにくい）。記事は切った断面が主題なので断面語を先に。
    # 2026-09-23 実測: 「白菜 断面」は当たらず（2語）、「ハクサイ」が**白菜と豆腐のスープ**に当たった。
    #   煮えた白菜は形も断面も見えないので、**丸のまま／切った株**が来る語を先に置く。
    #   `napa cabbage` は Commons のファイル名で実際に使われている語。
    ("白菜", ["napa cabbage", "Chinese cabbage", "白菜", "ハクサイ"]),
    #   カメムシ＝家に入る種は複数あり記事でも種を特定していない。学名も並べて候補を広げる。
    #   ただし**海外の侵入種としての写真**が来る可能性があるので、採用前に撮影地を見る（R30）。
    # 2026-09-23 実測: 「Halyomorpha halys」が**幼虫（nymph・飼育下）**に当たった。
    #   翅が無く成虫と見た目が違ううえ、記事は秋に家へ入る成虫の話。**成虫が来る語を先に**。
    #   `Plautia stali`（ツヤアオカメムシ）は日本で秋に家に入る代表種で、学名なら候補が出る。
    ("カメムシ", ["Plautia stali", "Halyomorpha halys adult", "カメムシ", "Pentatomidae"]),
