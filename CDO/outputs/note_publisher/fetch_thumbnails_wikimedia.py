...
JP_QUERY = [
    ...
    # 2026-09-23: 「タンス」「箪笥」は**別物に当たる**（過去に競馬場の写真、今回は路上ベンチの
    #   「桐タンス更生」広告看板＝業者の電話番号が読めるもの）。家具ではなく**衣類そのもの**に寄せる。
    # 2026-09-24 実測: 「folded clothes」が **骨董のハンガー（商標 Ideal/UNION が読める商品写真）**
    #   を拾った。Commons の "clothes" は**衣類用品**全般に届くので、ハンガー・ラック・什器が混ざる。
    #   衣替えの情景は「畳んで積んだ衣類」か「虫干し」なので、そちらの語に寄せる。
    # 2026-09-24 実測: 「虫干し」が **歌川国貞の浮世絵「虫干しの図」** に一致した（3枚目の却下）。
    #   古い風習の語は Commons では**版画の画題**として残っているほうが多い。写真が期待できる
    #   「押入れ」「収納」を先に置き、虫干しは最後に回す。
    # 2026-09-24 実測(3巡目): 「押入れ」が **旅館の客室（押し入れの中身は布団・手前に旅装のリュック）**
    #   に一致した。押し入れは寝具の収納として写るほうが多い。記事の [写真①] は
    #   「衣類を入れ替えた押し入れ（畳んだ服）」なので、**畳んだ衣類そのもの**の語に寄せる。
    ("衣替え", ["畳んだ 洗濯物", "衣類 収納", "folded laundry stack", "セーター 収納", "たとう紙"]),
    ...
    ("親芋", ["里芋 収穫", "里芋 畑"]),
    # 2026-09-23: 「疏水」が**琵琶湖疏水記念館**（京都の建物）に当たった。施設名を拾う語なので外す。
    # 2026-09-24 実測: 「irrigation canal」「水路」が **地中海の港町の運河（夜景・ヨット）** に一致した。
    #   canal は運河も指すので、船の泊まる水路に届く。**田や町なかの流れ**と分かる語に寄せる。
    # 2026-09-24 実測(2巡目): 「農業用水路」が **インドネシアの棚田（夕焼け）** に一致した。
    #   1巡目は地中海の運河で、2回とも日本ですらない。記事は「町の中を速い水が流れている」＝
    #   **コンクリートで護岸された町なかの流れ**。日本語＋構造物の語で固める。
    ("用水路", ["用水路 コンクリート", "農業用水路 日本", "用水 護岸", "水路 住宅地"]),
    ("速い水が流れている", ["疏水", "用水路 日本"]),
    ...
    # 2026-09-24 実測: 「栗ご飯」が **駅弁の栗めし** に一致し、箸袋に店名・住所・電話番号が読めた。
    #   弁当は必ず容器と付属品が写るので、**器に盛った家庭の栗ご飯**の語に寄せる（駅弁は外す）。
    # 2026-09-24 実測: 「栗飯 茶碗」「chestnut rice bowl」がどちらも **外食の丼**（駅弁／うに丼）に
    #   一致し、2回とも**箸袋の店名**が読めた。外食の写真は容器と付属品に店名が入るので避ける。
    #   栗そのものに寄せる（記事は栗を甘くしない食べ方の話なので、いがぐり・むき栗でも合う）。
    ("栗ご飯", ["栗ご飯", "栗飯", "いがぐり", "chestnut japanese"]), ("栗", "栗"),
    ("新米", "白米"), ("白米", "白米"),
    ...
    #   題材が一意に決まる語だけを並べる。
    # 2026-09-23 第2弾。**8月の暮らし系13本に検索語が1つも無かった**ので取得が空振りしていた。
    #   人物が写りやすい題材（日焼け・汗・高校野球・水遊び）は、**人ではなく物・場所に寄せる**
    #   （麦わら帽子／うちわ／無人の野球場／道具そのもの）。A4・肖像の3型に触れないため。
    # 2026-09-23 第2弾の実測: **抽象的・体験的な題材は、物で代用すると別物に当たる**。
    #   13本取って採用できたのは2本だけだった。失敗の型＝
    #     「たらい」→**みたらい渓谷**（部分一致）／「麦わら帽子」→**ゴッホの油彩**（絵は不可）／
    #     「水鉄砲」→英語パッケージの玩具／「うちわ」→**屋号とQRが読める**ホテルの名入り／
    #     「野球場」→**電話番号が読める**公衆便所／「提灯」→**奉納者の個人名が何十個も読める**／
    #     「weed control」→**人物の顔が写った米国の白黒アーカイブ**。
    #   → 物で代用するなら **その物だけが大きく写る語** を選ぶ。場所・施設・商品名は避ける。
    ("早期退職", ["office chair", "空のオフィス", "デスク"]),
    ("高校野球", ["スコアボード", "baseball scoreboard", "野球 ボール"]),
    # 朝顔の観察日記は日本の夏休みの宿題の定番。物として一意に決まる。
    ("夏休みの宿題", ["朝顔", "morning glory flower", "ドリル 学習"]),
    ("昼寝", ["和室", "tatami room", "座布団"]),
    ("草むしり", ["雑草", "軍手", "草取り鎌"]),
    ("日焼け", ["日傘", "parasol", "麦わら帽子 写真"]),
    ("寝苦しい夜", ["扇風機", "electric fan", "枕"]),
    # 「たらい」は**みたらい渓谷**に部分一致する。語を変える。
    ("行水", ["洗濯盥", "ブリキ たらい", "washtub"]),
    ("水鉄砲", ["water pistol", "みずでっぽう", "水遊び 子ども無し"]),
    ("汗", ["扇子", "folding fan japan", "団扇 無地"]),
    # 提灯は**奉納者の個人名が書かれている**ことが多い。蚊帳を先に。
    ("怪談", ["蚊帳", "mosquito net japan", "線香"]),
    ("夕涼み", ["縁側", "engawa", "夕涼み"]),
    # 2026-09-24 実測: 先頭が「風鈴」だったので**軒の風鈴**が来た。だが風鈴は
    #   「2026-08-01_風鈴_音で涼む夏」という**別記事の題材**で、そちらに使うべき写真。
    #   夏バテの記事は「だるさとつきあう」話なので、暑さをしのぐ道具のほうに寄せる。
    ("夏バテ", ["麦茶 グラス", "扇風機", "うちわ", "冷たい 麦茶"]),
    # 2026-09-23 取りこぼし回収分。公開済みで無サムネだった記事のうち、検索語が無かったもの。
    #   「二百十日」「FIRE」はCommonsに当たる語ではないので、**記事が見せている物**に寄せる。
    # 2026-09-24 実測: **冬の校庭のスケートリンク**が来た（右下に撮影日の焼き込みつき）。
    #   「台風」は気象衛星画像・被害の記録・季節外の写真に広く当たるので外す。
    #   記事は「稲が実る頃に来る風への備え」なので、実った稲そのものに寄せる。
    ("二百十日", ["稲穂", "水田 稲穂", "田んぼ 稲", "稲 実り"]),
    ...
    #   ブランド名が大きく読める）。容器ではなく**中身・道具**に寄せる。
    ...
    # ── 2026-10-03 の新規2本。
    # 湯豆腐＝**鍋の中の豆腐と昆布**。店の鍋は器に店名が入りやすいので「料亭」は入れない。
    # 2026-09-25 実測: 「湯豆腐」が **白菜・えのき・春菊の入った土鍋** に一致した。
    #   店で出る湯豆腐は野菜が入っていることが多く、**記事の主題（豆腐しかない）と食い違う**。
    #   鍋ではなく**豆腐そのもの**に寄せる。
    ("湯豆腐", ["豆腐", "木綿豆腐", "絹ごし豆腐", "豆腐 一丁"]),
    ("豆腐", ["豆腐", "木綿豆腐", "tofu block"]),
    # 長靴＝**玄関に置かれたゴム長靴**。**表札が写るものは不可**（玄関の写真は個人名が入りやすい）
    #   なので「玄関」は入れず、長靴そのものの語で引く。
    # 2026-09-25 実測: 「rubber boots」が **ヴェネツィアの土産物店の使い捨てシューズカバー**
    #   （商標とコカ・コーラの広告が読める）に一致した。英語の語は観光地の売り物に届く。
    #   日本語の語だけにする。
    ("長靴", ["ゴム長靴", "長靴", "雨靴"]),
    # 2026-09-24: 富山ブラックの記事を**「家で作ると黒くならない／黒さは醤油の選び方」**に書き直した。
    #   旧サムネ（File:Ramen Bowl 2）は「スープが澄んでいて富山ブラックではない」として却下済み。
    #   店の一杯を正しく引くのは難しい（色が店ごとに違う＝何が正解か画像で決められない）ので、
    #   **書き直した切り口の題材＝醤油そのもの**に寄せる。**銘柄ラベルが読めるものは却下型②**。
    # 2026-09-24 実測: 「醤油 小皿」が **折り紙で作った寿司と醤油皿** に一致した（工作物）。
    #   「小皿」「dish」は器の語なので、器に何か載った工作・食品サンプルにも届く。
    #   **液体としての醤油**に寄せる。瓶は銘柄ラベルが写るので入れない（却下型②）。
    # 2026-09-24 実測(3巡目): 「soy sauce」が **黄色いスープの麺料理に醤油を注ぐ写真**（東南アジア）
    #   に一致した。醤油は単体では絵にならず、必ず何かに注がれている写真になる。
    #   **題材の選び方を変える**＝記事自身が「茶色で止まる」と書いているので、
    #   **ふつうの醤油ラーメン（茶色）** がいちばん正直な絵になる。富山ブラックだとは言わない。
    #   1巡目=折り紙の工作物／2巡目=同左／3巡目=東南アジアの麺。日本語の語に寄せる。
    ("富山ブラック", ["醤油ラーメン", "中華そば", "ラーメン 醤油", "shoyu ramen"]),
    ("醤油", ["soy sauce", "醤油", "たまり醤油"]),
    # 2026-09-24 追加: 共働きの記事に**茅葺きの古民家**が来た＝検索語が無く題材から遠い写真を拾っていた。
    #   本文の [写真①] は「庭のついた大きな二階建ての持ち家」＝**現代の住まい**。
    #   ※表札に個人名が読める写真は却下型④なので、採用時に必ず開いて確認する。
    ("共働き", ["日本 住宅地", "郊外 住宅 日本", "detached house japan", "一戸建て 住宅"]),
    # ── 2026-10-02 の新規2本。
    # きのこ＝記事は**売り場に並ぶ栽培きのこ**の話。山で採る天然きのこの写真は主題が違う
    #   （本文は「見分け方は書かない」と明言している）。栽培・パック・棚の語に寄せる。
    #   **店名や商標が読めるものは不可**なので「スーパー」は入れない（看板が写る）。
    # 2026-09-24 実測: 「ぶなしめじ」が **おやき（きのこ入りの菓子・店名つき）** に一致した。
    #   きのこの和名は**料理名の一部**として使われるので、Commons では具材の写真に届く。
    #   学名と英名を先に置く＝栽培きのこそのものの写真に寄せる。
    ("きのこ", ["Hypsizygus marmoreus", "Pleurotus eryngii", "enoki mushroom",
              "mushroom cultivation", "Grifola frondosa"]),
    ("えのき", ["Flammulina velutipes", "enoki mushroom"]),
    ("しめじ", ["Hypsizygus marmoreus", "mushroom cultivation"]),
    # 朝霧＝**川や田んぼに低く溜まった霧**。山の雲海（上から見下ろす構図）は別物で、
    #   記事は窓から見た低い霧の話。「雲海」は入れない。
    # 2026-09-24 実測: 「morning fog river」が **フリードリヒの油彩（エルベ川の船）** に一致した。
    #   霧は画題として有名なので、短い語だと絵画に届く。**場所の語を足して**写真に寄せる。
    # 2026-09-24 実測(2巡目): 「morning fog rice field」が **フィリピンの水田**に一致した。
    #   英語の一般語は熱帯の水田に届くので、**日本語の語を先に**置く（1巡目は英語で絵画に届いた）。
    ("朝霧", ["朝霧 田んぼ", "川霧", "霧 田園 日本", "朝靄", "morning mist japan countryside"]),
    ("霧", ["fog over paddy field", "river mist japan", "朝霧 田んぼ"]),
    ("むかご", ["むかご", "零余子", "Mukago", "Dioscorea japonica"]),
    # 2026-09-25 実測: 「稲 再生」は pages:1 / too_small:1。同じく単語に寄せる。
    # 2026-09-23: 「稲孫」は**人名 錢稻孫**に当たり、1942年の北京大学の写真が降ってきた（2度目）。外す。
    # 2026-09-24 実測: 「ratoon rice」が **収穫前の実った稲穂の田**（頭を垂れた穂・熱帯の景色）
    #   を拾った。記事は「刈り終えた田んぼがもう一度緑になる」＝**刈り株からの再生**なので、
    #   同じ稲でも**成長段階が違う＝別物**（CLAUDE.md の却下型）。穂ではなく**刈り株**の語を先に置く。
    ("二番穂", ["rice stubble", "稲刈り後 田", "ひこばえ", "二番穂", "harvested paddy field"]),
    ("ひこばえ", ["ひこばえ", "二番穂", "ratoon rice"]),
    ("田んぼ", "田んぼ"), ("稲", "田んぼ"), ("水田", "田んぼ"),
    ("たこ焼き", "たこ焼き"), ("たこ焼", "たこ焼き"),
    ("麦わら帽子", "麦わら帽子"), ("麦藁帽子", "麦わら帽子"),
    ("焼きそば", "焼きそば"), ("焼そば", "焼きそば"),
    ("うちわ", "うちわ"), ("団扇", "うちわ"),
    # 2026-09-23: 「獅子舞」は**人物（子どもを含む）が写る写真**ばかりになる。踊り手ではなく
    #   **獅子頭そのもの**を先に狙う。人物が写るものは題材が合っていても使わない（A4・肖像）。
    # 2026-09-24 実測: 3巡して3回とも駄目だった。①サンフランシスコの中国獅子舞（舞獅）
    #   ②子どもの顔が写るもの ③**三重の獅子頭**（R30 が検知）。
    #   記事は「富山の秋は笛と太鼓で始まる」＝**富山の獅子舞が主題**で、獅子舞は地域で形が違う。
    #   障子（どこの障子でも同じ）とは事情が違うので、他県のものは使わない。土地の語を必ず入れる。
    #   ※これでも取れないときは、語ではなく**記事側の[写真]指定**を見直す（無サムネにはしない）。
    ("獅子舞", ["富山 獅子舞", "百足獅子", "越中 獅子舞", "獅子舞 富山県"]),
    ("秋祭り", ["獅子頭", "神輿", "祭り 提灯"]),
...
NON_PHOTO_HINTS = (
    ...
    # ★2026-09-24 追加: 実測2件がここを素通りした。
    #   ①`Elbschiff im Fruehnebel (C D Friedrich)` ＝カンヴァスの亀裂が見える油彩（朝霧の記事）
    #   ②`NDL-DC 1301948-Utagawa Kunisada-〔虫干しの図〕` ＝浮世絵の三枚続（衣替えの記事）
    #   画家名を1人ずつ足すのは切りがないので、**出所と版種の語**も入れる。
    #   `ndl-dc` は国立国会図書館デジタルコレクション＝古書・版画のスキャンで、写真ではない。
    "friedrich", "rembrandt", "vermeer", "turner", "constable", "renoir", "cezanne",
    "utagawa", "kunisada", "hiroshige", "kuniyoshi", "kitagawa",
    "ndl-dc", "浮世絵", "錦絵", "木版", "woodblock", "の図", "三枚続",
    # ★2026-09-24 追加: **食べ物に見えるが作り物**。実例＝`Sushi on the dish and Soy sauce of
    #   Japanese Origami Papers`（折り紙の寿司と、赤茶の色紙で作った醤油皿）が醤油の記事に来た。
    #   食品サンプルや手芸の再現品は食の記事に紛れ込みやすく、A3＝写真風で統一にも反する。
    "origami", "折り紙", "食品サンプル", "replica", "felt food", "papercraft", "ペーパークラフト",
)
...
UNUSABLE_NAME_HINTS = (
    ...
    "ホテル", "旅館", "hotel ", "ryokan",
    # ★2026-09-24 追加: 衣類まわりの**什器・用品**。衣替えの記事に骨董ハンガーの商品写真が来た
    #   （木部に商標 Ideal / UNION が読める）。衣類そのものでなく、それを掛ける道具の写真。
    "コートハンガー", "ハンガー",
    # ★2026-09-24 追加: ファイル名に**店名**が入る写真。
    #   実例＝`おやき（ねぎみそ、ぶなしめじ）（いろは堂MIDORI長野店）` ＝きのこの記事に来た菓子。
    #   店名が読める写真は却下型②なので、名前の時点で落とす。
    #   記事の題材に「店」が入るとき（商店街など）は既存のガードで弾かれない。
    "店）", "店)", "本店", "支店",
)

# 英字の語は**語の一部に埋まっている**ことがあるので、単純な部分一致では誤爆する。
# 実測: "hanger" を部分一致にすると `Hangerford bridge` まで落ちた。ここは語境界で見る。
UNUSABLE_NAME_WORDS = (
    "hanger", "hangers", "clothes rack", "coat rack",
)
...
def _looks_unusable_name(title: str, topic: str = "") -> bool:
    ...
    for h in UNUSABLE_NAME_HINTS:
        if h in t and h not in tp:
            return True
    for w in UNUSABLE_NAME_WORDS:
        if re.search(rf"\b{re.escape(w)}\b", t) and w not in tp:
            return True
    return False
...
NON_JAPAN_HINTS = (
    ...
    # 2026-09-24 実測: 朝霧の記事に `File:Rice fields at Baliuag`（フィリピン）が来た。
    #   国名は入っておらず**町の名前だけ**だったので、ここの語では止まらなかった。
    #   地名を1つずつ足しても追いつかないので、**日本固有の題材は日本語の語で引く**のを基本にし、
    #   ここは実際に来たものを記録する場所として使う（同じ町名が二度来ないように）。
    "baliuag", "luzon", "bali", "java", "kerala", "tamil",
    # 2026-09-25: 長靴の記事にヴェネツィアの土産物（使い捨てシューズカバー）が来た。
...
# ★2026-09-24: **土地名の要求を外す記事**を、理由つきで明示する。
#   このガードの趣旨は「固有名詞の記事に**全国の代表例**を拾わせない」こと
#   （富山ブラックの記事に、ただのラーメンを持ってこない）。
#   だが記事によっては、**見出し画像の題材をわざと別のものに移してある**ことがある。
#   その場合ここが全部のクエリを捨ててしまい、**取得が一度も走らない**（実測でそうなった）。
#   抜け道を黙って作らず、記事ごとに理由を書いて残す。
PLACE_EXEMPT = {
    # 記事の見出し画像を「店の一杯」ではなく **醤油そのもの** にした（2026-09-24 に書き直し）。
    # 店の黒さは店ごとに違い、画像で正解を決められない。旧サムネ（Ramen Bowl 2）は
    # 「スープが澄んでいて富山ブラックではない」として却下済み。
    "富山ブラックラーメン_労働者の塩分補給": "見出し画像の題材を醤油に移した記事",
}


def _place_exempt(stem: str) -> str:
    for key, why in PLACE_EXEMPT.items():
        if key in (stem or ""):
            return why
    return ""
