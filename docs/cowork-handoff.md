...
- **復旧（6/12）**：ops/ 一式・本ファイル・cowork設計書3本・サムネ生成マスターを `14f4eda` から復元。
  未復旧分（workflows 4本・CAO戦略/監査・CDOその他）は要オーナー判断（`git checkout 14f4eda -- <path>` で随時復元可）。
- **指示の現況**：inbox の過去指示は全て `superseded`。**有効なのは `ops/inbox/2026-06-12_002_code_cowork.yaml` のみ**。
...
- **6/12 ストック再配分**：記事11本のファイル名日付を変更（EN素材9ファイル連動）。最新構成は `context/STATE.md`。
