...
push CMO/outputs/** または 手動 workflow_dispatch
  └─ Step1: fetch_thumbnails_wikimedia.py … Wikimedia Commons から実写取得（キー不要・完全無料・既定）
  └─ Step2: fetch_note_thumbnails.py      … PEXELS_API_KEY があるときだけ、不足分をフリー写真で補完
  └─（有料AI生成 generate_thumbnails.py は撤去＝¥0原則A2/A3。Picsumランダムも廃止）
  └─ Commit: git add -f thumbnails/*.jpg と _provenance.json → commit → push
```

### 画像ソースの優先順（すべて無料・¥0原則A2/A3）

| 優先 | ソース | 条件 | 備考 |
|------|------------|------|------|
| 1 | **Wikimedia Commons** | キー不要（既定） | 完全無料。記事題材の実写を取得。失敗分は次回トリガーで再取得（自己修復） |
| 2 | Pexels フリー写真 | `PEXELS_API_KEY`（無料）あり | Step1 で埋まらなかった分のみ実写で補完 |

> ★有料の画像生成AI（OpenAI/Gemini Imagen/Pollinations 等）はワークフローから撤去済み。
> 課金の発生源を持たない＝¥0。`generate_thumbnails.py` はリポジトリに残るが**どのワークフローからも呼ばれない**（無効）。
