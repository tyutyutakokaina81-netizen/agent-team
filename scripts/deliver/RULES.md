...
## 🆕 本番実行で追加されたルール（2026-04-21）

### ルール7: 破壊的操作の前に既存確認

**既存ファイルがある場合、問答無用で上書きしない：**

1. 対話型 → 確認プロンプト（`上書きしますか？ (y/N)`）
2. 自動実行 → スキップ＋警告
3. バッチ処理 → バックアップしてから上書き
4. `--force` オプション指定時のみ強制上書き

**理由：** 手動で改善した成果物が再実行で消えるリスク

### ルール8: 納品物の明示分離

**クライアントに納品するファイルを明確に区別する：**

1. `meta.json` の `main_deliverable` フィールドで明示指定（最優先）
2. 命名規則で自動判定（`article.md`, `delivery.md`, `output.md`, `main.md`, `final.md`）
3. 番号プレフィックス付き（`1_structure.md`, `2_notes.md` 等）は**作業ファイル扱い**で納品対象外

**理由：** 作業中の構成メモをクライアントに誤って送信しない

### ルール9: ユーザー情報の一元管理

**個人情報（名前・メール等）は `user_config.json` で管理：**

1. 保存先: `~/agent-team/user_config.json`（.gitignore対象）
2. 権限: 0600（本人のみ読み書き）
3. テンプレ展開時に `apply_placeholders()` で自動置換
4. 未設定なら placeholder のまま残す（明示的に見える）

**理由：** 個人情報の散在防止・毎回の手動置換を排除
...
**理由：** デモを実行して本番データが消える事故を防ぐ

### ルール11: 冪等性 ≠ 再生成性

**2つの概念を区別する：**

- **冪等性（Idempotent）：** 何度実行しても結果が同じ状態に収束 ✅ 必須
- **再生成性（Regenerative）：** 既存結果を無視して毎回再生成 ❌ 避ける

**デフォルト動作は「既存尊重」、明示フラグで「再生成」。**

### ルール12: 多段階テストの実施

**新機能追加時は以下の3段階でテスト：**

1. **新規実行** — 何もない状態からの初回動作
2. **再実行** — 既存データがある状態での2回目動作（冪等性確認）
3. **部分再開** — 途中まで進んだ状態からの再開（ステージ検出確認）
...
- **2026-04-21 初回作成**：4原則＋12ルール
- **2026-04-21 本番実行追加**：ルール7-12（既存ファイル保護・ユーザー情報管理等）
