...
# 使い方: ./smoke_test.sh
#         サーバーを別タブで起動済みであること
#         pipeline 認証ありをテストする場合: PIPELINE_TOKEN=xxxx ./smoke_test.sh
...
# ─────────────────────────────────────────────
# check_status <label> <url> <expected_code> [curl args...]
# 例: check_status "POST /echo" http://... 200 -X POST -d '{...}'
# ─────────────────────────────────────────────
check_status() {
  local label="$1"
  local url="$2"
...
# ─────────────────────────────────────────────
# check_jsonkey <label> <url> <key> [curl args...]
# JSON レスポンスに指定キーがあるか
# ─────────────────────────────────────────────
check_jsonkey() {
  local label="$1"
  ...
  local key="$3"
  shift 3
  if curl -s "$@" "$url" 2>/dev/null \
       | python3 -c "import sys,json; d=json.load(sys.stdin); sys.exit(0 if '$key' in d else 1)" \
       2>/dev/null; then
...
check_status  "POST /echo 不正 JSON → 400" "http://127.0.0.1:$GATEWAY_PORT/echo" 400 \
...
if [[ -n "$PIPELINE_TOKEN" ]]; then
  check_status "POST /search 認証あり → 200" "http://127.0.0.1:$PIPELINE_PORT/search" 200 \
